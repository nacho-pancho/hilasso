...
% 
function buffer_info = mdlsCreatePatchBufferAligned(image_dir, ...
                                     image_list, ...
                                     patch_width, ...
                                     buffer_prefix)
    remove_dc = true;
    ...
    ang_name = [buffer_prefix '-ang.bin'];
    fang = fopen(ang_name,'w');
    [em,en]=ferror(fang);
    if (en ~= 0) 
        error(['Error opening ' ang_name ' for writing: ' em]);
    end    
    byte_count = 0;
    patch_count = 0;
    w = patch_width;
    ...
    for i = 1:NI
        ...
        data        = mdlsGetDataAligned(I,w,overlap);
        DC = data.DC;
        X = data.X;        
        ang = data.angle;
        ...
        fwrite(fang,ang,'single');
        [em,en]=ferror(fang);
        if (en ~= 0) 
            error(['Error writing to ' buffer_prefix '-ang.bin: ' em]);
        end
        %
