...
% rev   : $Revision$
%
function [k,b,u]=mdlsMOLFit(X,k0,method)
%
...
%
% (non-central) moments of order j are infinite for j >= k and 
% mu^j = b^j/(k-1 choose j) for j < k
% in particular, mu=b/(k-1) and sigma^2=2b^2/[(k-1)(k-2)]
% which yields 
% k=2(sigma^2 - mu^2)/(sigma^2 - 2mu^2)
% and b = (k-1)mu
%
[m,n]=size(X);
if (m > 1) && (n == 1)
