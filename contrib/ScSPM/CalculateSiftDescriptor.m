function [database, lenStat] = CalculateSiftDescriptor(rt_img_dir, rt_data_dir, gridSpacing, patchSize, maxImSize, nrml_threshold,useMask)
%==========================================================================
...
%         if ii == length(subfolders)-1; useMask=1;end
%         if ii == length(subfolders); useMask=0;end
...
%             % Load bike mask (just take 1 bike per image
%             maskpath = [imgpath(1:end-9) 'mask.0.png'];
%             Im = imread(maskpath);
%             Im = double(Im);
% 
%             Im1 = sum(Im,2);
...
%             idx1 = find(Im1~=0);
%             idx2 = find(Im2~=0);
%             roi = [idx2(1) idx2(end) idx1(1) idx1(end)]';
            ...
            fpath = fullfile(rt_data_dir, subname, [fname, num2str(patchSize) ,'.mat']);
