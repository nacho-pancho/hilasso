function [x,x_debias,objective,times,debias_start,mses,taus]= ...
    SpaRSAalt(y,A,tau,p,c,a,varargin)

% SpaRSA version 2.0, December 31, 2007
% 
% This function solves the convex problem 
%
% arg min_x = 0.5*|| y - A x ||_2^2 + p'*x + c*0.5*|| a -  x ||_2^2 + tau phi(x)
%
...
%
%  w: 1D vector, the size of A*x that satisfies: w=p+ca (scalar)
%
%  ===== Optional inputs =============
%
%  
...
while keep_continuation 
  ...
  % for this tau
  alpha = 1.0;
  f = 0.5*(resid(:)'*resid(:)) + p'*x + c*0.5*(a-x)'*(a-x) + tau * phi_function(x); % 
  if enforceSafeguard
    f_lastM = f;
  end
  ...
  while keep_going
	...
	if verbose
	  fprintf(1,' f=%10.6e, increasing alpha to %6.2e\n', f, alpha);
          if alpha > eps^(-0.5)
              fprintf(1, 'alpha grew too much. Stopping.\n');
              nacho_says_stop = true;
              break;
          end
	end
      end
