function [x,x_debias,objective,times,debias_start,mses,taus,seq]= ...
...
while keep_continuation 
  ...
  while keep_going
	...
	cont_inner=0;
          ...
          if alpha > eps^(-0.5)
             % warning(1, 'alpha grew too much. Stopping.\n');
              nacho_says_stop = 1;
              break;
          end
      end
    ...
    switch stopCriterion
        ...
        case 5,
            ...
            delta_x_criterion = sqrt(dx(:)'*dx(:))/(x(:)'*x(:)+eps);
            keep_going = (delta_x_criterion > tolA);
            if verbose
                fprintf(1,'norm(dx)/norm(x) = %e (target = %e)\n',...
